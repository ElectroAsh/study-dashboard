...
export default function Design3({ events: initialEvents, handoff }) {
  ...
  const [tasks, setTasks] = useState((handoff.tasks||[]).map((t,i) => ({ id: i, text: t, done: false, subject: "", priority: "normal" })));
  ...
  const [sidebarTab, setSidebarTab] = useState("today"); // today | tasks | flags
  ...
  const todayEvts = getEvts(todayStr);
  const completedTasks = tasks.filter(t => t.done).length;
  const totalTasks = tasks.length;

  const addTask = () => {
    if (!newTask.trim()) return;
    setTasks(prev => [...prev, { id: Date.now(), text: newTask.trim(), done: false, subject: "", priority: "normal" }]);
    setNewTask("");
    ...
    <div style={{ background: "#f8fafc", minHeight: "100vh", color: "#0f172a", fontFamily: "'Inter',system-ui,sans-serif" }}>
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid #e2e8f0", padding: "0 28px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 52, position: "sticky", top: 56, zIndex: 90 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <button onClick={() => { setAddEventOpen(true); }} style={{ background: "#0f172a", color: "#fff", border: "none", borderRadius: 10, padding: "7px 14px", fontSize: 12, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
            + Add Event
          </button>

          <div style={{ fontSize: 12, color: "#94a3b8", fontWeight: 600 }}>
            {today.toLocaleDateString("en-AU", { day: "numeric", month: "short" })}
          </div>
        </div>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", minHeight: "calc(100vh - 108px)" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            ...
            <div style={{ display: "flex", gap: 6 }}>
              {assessments.slice(0,3).map((a,i) => (
                <div key={i} onClick={() => { setSelected(a.date); setMonth(new Date(a.date+"T12:00:00").getMonth()); }} style={{
                  background: urgencyBg(a.daysLeft), border: `1px solid ${urgencyColor(a.daysLeft)}33`,
          ...
          {view === "month" && (
            <>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 0, marginBottom: 0 }}>
                {DAYS.map(d => (
                  <div key={d} style={{ textAlign: "center", fontSize: 10, color: "#cbd5e1", fontWeight: 700, letterSpacing: 1, padding: "6px 0", borderBottom: "1px solid #e2e8f0" }}>{d}</div>
                ))}
              </div>
                ...
                {cells.map((day, idx) => {
                  ...
                  return (
                    <div key={dateStr} onClick={() => setSelected(dateStr)}
                      ...
                      style={{
                        minHeight: 90, borderRight: "1px solid #e2e8f0", borderBottom: "1px solid #e2e8f0",
                        padding: "6px 6px 4px", cursor: "pointer", position: "relative",
                        background: isSel ? "#f0f9ff" : isToday ? "#fefce8" : hoveredDay===dateStr ? "#f8fafc" : "#fff",
                        transition: "background 0.1s",
                      }}>
                      {/* Day number */}
                      <div style={{
                        width: 24, height: 24, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center",
                        background: isToday ? "#0f172a" : isSel ? "#3b82f6" : "transparent",
                        marginBottom: 4,
                      }}>
                        ...
                        {evts.slice(0,3).map((e,i) => {
                          ...
                          return (
                            <div key={i} style={{
                              ...
                              background: e.type==="Assessment" ? "#fee2e2" : c.light || "#f1f5f9",
                              color: e.type==="Assessment" ? "#dc2626" : "#374151",
                              overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
                              borderLeft: `2px solid ${e.type==="Assessment"?"#ef4444":c.dot}`,
                            }}>{e.title.replace("⚠️ ","")}</div>
                          );
                        })}
            ...
            <>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 8 }}>
                {getWeekDates().map(dateStr => {
                  ...
                  const isSel = dateStr === selected;
                  return (
                    <div key={dateStr} onClick={() => setSelected(dateStr)} style={{ cursor: "pointer" }}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 4, minHeight: 200 }}>
                        {evts.map((e,i) => {
                          const c = col(e.subject);
                          return (
                            <div key={i} style={{ padding: "6px 8px", borderRadius: 8, background: e.type==="Assessment"?"#fee2e2":c.light||"#f1f5f9", borderLeft: `3px solid ${e.type==="Assessment"?"#ef4444":c.dot}` }}>
              ...
              {selEvts.length === 0
                ? (
                  <div style={{ textAlign: "center", padding: "60px 0", color: "#cbd5e1" }}>
                    ...
                    <div style={{ fontSize: 13, marginTop: 4 }}>Free day — use it wisely</div>
        ...
        <div style={{ borderLeft: "1px solid #e2e8f0", background: "#fff", display: "flex", flexDirection: "column", height: "calc(100vh - 108px)", position: "sticky", top: 108, overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", borderBottom: "1px solid #e2e8f0" }}>
            {[{id:"today",label:"Today"},{id:"tasks",label:"Tasks"},{id:"flags",label:"Flags"}].map(tab => (
              <button key={tab.id} onClick={() => setSidebarTab(tab.id)} style={{
                flex: 1, padding: "12px 0", border: "none", cursor: "pointer", fontSize: 11, fontWeight: 700,
          ...
          <div style={{ flex: 1, overflow: "auto", padding: "16px" }}>
            ...
            {sidebarTab === "today" && (
              <div>
                <div style={{ fontSize: 12, fontWeight: 700, color: "#cbd5e1", letterSpacing: 2, textTransform: "uppercase", marginBottom: 12 }}>
                  {new Date(selected+"T12:00:00").toLocaleDateString("en-AU", { weekday: "long", day: "numeric", month: "short" })}
                </div>

                {selEvts.length === 0
                  ? <div style={{ color: "#e2e8f0", fontSize: 13, textAlign: "center", padding: "20px 0" }}>Nothing scheduled</div>
                      ...
                      {selEvts.map((e,i) => {
                        ...
                        return (
                          <div key={i} style={{ padding: "10px 12px", background: isExam?"#fff5f5":"#f8fafc", border: `1px solid ${isExam?"#fecaca":"#e2e8f0"}`, borderLeft: `3px solid ${isExam?"#ef4444":c.dot}`, borderRadius: 10 }}>
                            ...
                            <div style={{ fontSize: 10, color: "#94a3b8", marginTop: 2 }}>{e.subject}{e.time&&` · ${e.time}`}</div>
                          </div>
                        );
                      })}
                    </div>
                ...
                {/* Priority focus */}
                <div style={{ fontSize: 12, fontWeight: 700, color: "#cbd5e1", letterSpacing: 2, textTransform: "uppercase", marginBottom: 10 }}>Priority Focus</div>
                <div style={{ background: "#fff5f5", border: "1px solid #fecaca", borderLeft: "3px solid #ef4444", borderRadius: 10, padding: 12, marginBottom: 8 }}>
                  <div style={{ fontSize: 9, color: "#ef4444", fontWeight: 800, letterSpacing: 1.5, marginBottom: 4 }}>#1 · {handoff.priority_days}D · {handoff.priority_subject?.toUpperCase()}</div>
                  <div style={{ fontSize: 11, color: "#374151", lineHeight: 1.6 }}>{handoff.priority_focus}</div>
                </div>
                {handoff.secondary_focus && (
                  <div style={{ background: "#fff7ed", border: "1px solid #fed7aa", borderLeft: "3px solid #f97316", borderRadius: 10, padding: 12 }}>
                    <div style={{ fontSize: 9, color: "#f97316", fontWeight: 800, letterSpacing: 1.5, marginBottom: 4 }}>#2 · {handoff.secondary_days}D · {handoff.secondary_subject?.toUpperCase()}</div>
                    <div style={{ fontSize: 11, color: "#374151", lineHeight: 1.6 }}>{handoff.secondary_focus}</div>
                  </div>
                )}

                {/* Assessments */}
                <div style={{ fontSize: 12, fontWeight: 700, color: "#cbd5e1", letterSpacing: 2, textTransform: "uppercase", margin: "16px 0 10px" }}>Countdowns</div>
                {assessments.map((a,i) => {
                  const urg = urgencyColor(a.daysLeft);
                  return (
                    <div key={i} onClick={() => { setSelected(a.date); setMonth(new Date(a.date+"T12:00:00").getMonth()); }} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 10px", background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 10, marginBottom: 6, cursor: "pointer" }}>
                      <div style={{ width: 34, height: 34, borderRadius: 8, background: urgencyBg(a.daysLeft), display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                        <span style={{ fontSize: 13, fontWeight: 800, color: urg, lineHeight: 1 }}>{a.daysLeft}</span>
                        <span style={{ fontSize: 7, color: urg, fontWeight: 600 }}>days</span>
                      </div>
                      <div style={{ minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 10, color: "#94a3b8" }}>{a.subject}</div>
                      </div>
                    </div>
                  );
                })}
              ...
              <div>
                ...
                <div style={{ display: "flex", gap: 6, marginBottom: 14 }}>
                  <input
                    ...
                    style={{ flex: 1, padding: "8px 10px", border: "1.5px solid #e2e8f0", borderRadius: 8, fontSize: 12, outline: "none", background: "#f8fafc", color: "#0f172a" }}
                  />
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {tasks.map(t => (
                    <div key={t.id} style={{ display: "flex", alignItems: "flex-start", gap: 8, padding: "8px 10px", background: t.done?"#f0fdf4":"#f8fafc", border: `1px solid ${t.done?"#bbf7d0":"#e2e8f0"}`, borderRadius: 10, group: true }}>
                      <div onClick={() => setTasks(prev => prev.map(x => x.id===t.id?{...x,done:!x.done}:x))} style={{ width: 16, height: 16, borderRadius: 5, border: `2px solid ${t.done?"#22c55e":"#d1d5db"}`, background: t.done?"#22c55e":"transparent", flexShrink: 0, marginTop: 1, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.15s" }}>
                      ...
                      <button onClick={() => removeTask(t.id)} style={{ background: "none", border: "none", color: "#cbd5e1", cursor: "pointer", fontSize: 14, padding: "0 2px", lineHeight: 1 }}>×</button>
                    </div>
                  ))}
                  {tasks.length === 0 && <div style={{ textAlign: "center", color: "#e2e8f0", fontSize: 13, padding: "20px 0" }}>No tasks yet 🎉</div>}
                </div>
            ...
            {sidebarTab === "flags" && (
              <div>
                <div style={{ fontSize: 12, fontWeight: 700, color: "#cbd5e1", letterSpacing: 2, textTransform: "uppercase", marginBottom: 12 }}>Active Flags</div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 999, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setAddEventOpen(false)}>
          <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 20, padding: 28, width: 380, boxShadow: "0 20px 60px rgba(0,0,0,0.15)" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <input value={newEvent.time} onChange={e => setNewEvent(p=>({...p,time:e.target.value}))} placeholder="Time (optional, e.g. 1:00–3:00pm)" style={{ padding: "10px 12px", border: "1.5px solid #e2e8f0", borderRadius: 10, fontSize: 13, outline: "none", color: "#0f172a" }}/>
